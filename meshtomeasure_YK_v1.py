...
def resolve_landmarks_with_custom(default_landmarks: dict) -> dict:
    ...
    # Added 'Cz': 'CZ' to the mapping
    mapping = {'Fpz': 'FPZ', 'Oz': 'OZ', 'dlPFC': 'DLPFC', 'Cz': 'CZ'}
    ...
    for key_lmk, key_upper in mapping.items():
        ...
        if v is None:
            # Only Fpz, Oz, and dlPFC are strictly "essential" to trigger the override logic
            if key_upper != 'CZ': 
    ...
    if missing_essential:
        print("ESSENTIAL CUSTOM VARIABLES NOT FOUND!", file=sys.stderr)
        return default_landmarks
    else:
        return {
            'Cz': overrides.get('Cz', default_landmarks['Cz']), # Use custom Cz if provided
            'Fpz': overrides['Fpz'],
            'Oz': overrides['Oz'], 
