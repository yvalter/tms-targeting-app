...
def process_mri_segmentation_task(self, nifti_path, output_dir, session_id):
    ...
    try:
        ...
        # 4. NONLINEAR TARGETING
        logger.info("Starting Nonlinear Targeting for DLPFC...")
        target_mni = [(-38, 44, 26)] 
        
        # Call the updated helper function
        subject_coords = map_mni_to_subject_low_ram(nifti_path, target_mni)
        
        if subject_coords:
            tx, ty, tz = subject_coords[0]
            generated_files['DLPFC_coords'] = [tx, ty, tz]
            logger.info(f"DLPFC subject coords: {tx}, {ty}, {tz}")
